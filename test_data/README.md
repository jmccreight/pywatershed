...
The `run_prms_domains.py` scripts runs reference models, specifically the
PRMS v5.2.1 binary. That binary is not kept in the repository; it is
compiled from `prms_src/prms5.2.1` with gfortran the first time it is
needed and written to `bin/`. All
...
The PRMS binaries are compiled on demand by
`pywatershed.utils.compile_prms()`, which builds with gfortran and installs
into `bin/` under the name `pywatershed.utils.get_prms_exe_name()` expects.
To force a rebuild, call it with `force=True`. If a build fails, please see
[`prms_src/prms5.2.1/README.md`](../prms_src/prms5.2.1/README.md).

The GSFLOW binaries in `bin/` are the exception: GSFLOW source is not part
of this repository, so they are checked in. The Apple Silicon one is an
x86_64 intel build and needs Rosetta 2.
