...
from pywatershed.atmosphere.prms_atmosphere import PRMSAtmosphereTranspFrost
...
params = ["params_sep", "params_one"][1:]


@pytest.fixture(scope="function")
def control(simulation):
    if "transp_frost" not in simulation["name"]:
        pytest.skip(
            "Only testing PRMSAtmosphereTranspFrost for domains with "
            "'transp_frost' in the control pattern."
        )
...
def parameters(simulation, control, request):
    if request.param == "params_one":
        ...
        param_file = (
            simulation["dir"] / "parameters_PRMSAtmosphereTranspFrost.nc"
        )
...
def test_compare_prms(
    ...
    comparison_var_names = list(
        set(PRMSAtmosphereTranspFrost.get_variables()) - set(["tmax_sum"])
    )
    ...
    input_variables = {}
    for key in PRMSAtmosphereTranspFrost.get_inputs():
        if "soltab" in key:
